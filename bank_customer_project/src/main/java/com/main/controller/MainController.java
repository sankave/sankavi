...
{
	return new ModelAndView("WithinBankTransactionPage");
}
...
{
	return new ModelAndView("OutsideBankTransactionPage");
}


@RequestMapping("/WithinBank")
public ModelAndView WithinBank(HttpServletRequest request,@ModelAttribute("tran") Transaction tran)
{
	HttpSession session=request.getSession();
	Registration r1=(Registration) session.getAttribute("regis");
	serviceAcc.createAccWithinBank(request,r1.getId(),tran);
	return new ModelAndView("redirect:/savingsCheck");
}
